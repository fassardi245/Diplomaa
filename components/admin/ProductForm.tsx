...
export default function ProductForm({ categories, product, initialCategoryId }: ProductFormProps) {
  ...
  // --- Imagen ---
  // Intentamos obtener la imagen del objeto expandido o del array
  const initialImage = product?.imageUrl || (product?.images && product.images[0]?.asset?.url) || null;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Capturamos todos los datos del formulario, INCLUYENDO el archivo de imagen
    const formData = new FormData(e.currentTarget);

    try {
        // Ejecutamos la Server Action directamente.
        // La Action tiene 'backendClient' y permisos para subir la foto.
        if (isEditing) {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {/* Este input lleva el archivo al servidor */}
          ...
          <div className="lg:col-span-2 space-y-5">
              ...
              <div className="pt-4 flex justify-end">
                 <button 
                    type="submit" 
                    disabled={isLoading}
                    className="flex items-center gap-2 bg-black text-white px-8 py-3 rounded-xl font-bold hover:scale-105 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                 >
                    {isLoading ? (
                        <>
